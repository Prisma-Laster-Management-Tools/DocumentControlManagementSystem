...
export class ProductRepository extends Repository<Product> {
  ...
  async getAllProduct(paginationDto: PaginationDto): Promise<PaginateResult> {
    ...
    const SEARCH_CONDITION_RENDERED = `${
      paginationDto.search ? `%${paginationDto.search}%` : '%%'
    }`;
    ...
    const Plain_Products_Query_With_Pagination =
      Plain_Products_Query + ` ${PAGINATION_QUERY_STR}`;

    const [getCountPromise, Products] = await Promise.all([
      paginationDto.search
        ? new Promise(async (resolve, reject) => {
            const all_plain_products_with_no_pagination = await this.query(
              Plain_Products_Query,
            );
